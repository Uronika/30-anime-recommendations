...
import { createEmptyProfile, selectionName, type ChallengeProfile, type DailyEntry } from './domain/types'
...
export default function App() {
  ...
  async function importProfile(next: ChallengeProfile) { try { await challengeRepository.replace(next); setProfile(next); setNotice('备份已恢复。') } catch (cause) { setNotice(cause instanceof Error ? cause.message : '备份不兼容。') } }
  ...
  <section className="editor-shell"><div className="day-heading"><span>DAY {String(day.day).padStart(2, '0')}</span><h2>{day.title}</h2><p>{day.kind === 'character' ? '选择一个让你念念不忘的角色。' : day.kind === 'music' ? '写下那首一响起就回到故事里的歌。' : '从 Bangumi 数据库选一部动画，或手工记录。'}</p></div><EntryEditor day={day} entry={entry} onChange={updateEntry} /><div className="editor-footer"><button type="button" className="secondary" disabled={currentDay === 1} onClick={() => setCurrentDay((value) => value - 1)}>上一题</button><span>{selectionName(entry.selection) ? '已自动保存' : '等待你的选择'}</span><button type="button" className="secondary" disabled={currentDay === 30} onClick={() => setCurrentDay((value) => value + 1)}>下一题</button></div></section>
