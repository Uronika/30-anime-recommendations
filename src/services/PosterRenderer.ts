...
import { displayArtwork } from './BangumiImageProxy'
...
export class PosterRenderer {
  ...
  async render(profile: ChallengeProfile): Promise<HTMLCanvasElement> {
    ...
    const imageEntries = profile.showCovers ? cards.map((card) => displayArtwork(card.entry.selection)?.imageUrl).filter((url): url is string => Boolean(url)) : []
    ...
    for (let row = 0; row < 6; row += 1) {
      ...
      for (const card of rowCards) {
        ...
        if (profile.showCovers) {
          ...
          const artwork = displayArtwork(card.entry.selection)
